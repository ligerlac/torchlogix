...
from models.baseline_nn import FullyConnectedNN
...
def get_model(args):
    ...
    if arch == 'randomly_connected':
        model = RandomlyConnectedNN(in_dim, k, l, class_count, tau, **llkw)
    elif arch == 'fully_connected':
        model = FullyConnectedNN(in_dim, k, l, class_count, dtype)
    elif arch == 'cnn':
