...
class CNN(torch.nn.Module):
    def __init__(self, class_count, tau, **llkw):
        ...
        k_num = 16
        logic_layers.append(LogicConv3d(in_dim=28, num_kernels=k_num, channels=1, **llkw, tree_depth=3,
                                          receptive_field_size=5, padding=0))
        logic_layers.append(OrPoolingLayer(kernel_size=2, stride=2, padding=0))

        logic_layers.append(LogicConv3d(in_dim=12, channels=k_num, num_kernels=3 * k_num, **llkw, tree_depth=3,
                                          receptive_field_size=3, padding=0))
        logic_layers.append(OrPoolingLayer(kernel_size=2, stride=2, padding=1))

        logic_layers.append(LogicConv3d(in_dim=6, channels=3 * k_num, num_kernels=9 * k_num, **llkw, tree_depth=3,
                                          receptive_field_size=3, padding=0))
        logic_layers.append(OrPoolingLayer(kernel_size=2, stride=2, padding=1))
