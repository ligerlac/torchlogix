...
class _LogicConvNd(LogicBase):
    ...
    def _init_weights(self):
        ...
        # Store weights in merged format: (num_kernels * lut_rank**i, weight_dim)
        # This eliminates the need for permute+reshape in forward pass
        tree_weights = torch.nn.ParameterList()
        for i in reversed(range(self.tree_depth)):
            n = self.lut_rank ** i

            level_weights = self.parametrization.init_weights(
                self.num_kernels * n,
                self.device
            )
    ...
    def forward(self, x):
        ...
        # x is of shape (batch_size, lut_rank, spatial_positions, K*F) - already merged by connections

        # first level tree processing (from receptive field)
        # weights are already in merged format: (K*F, weight_dim)
        ...
        for level in range(1, self.tree_depth):
            ...
            x = x.view(*x.shape[:2], self.num_kernels, -1)

            # Apply connections - returns (B, lut_rank, S, K*M) with merging already done
            x = self.connections(x, level)

            # Apply parametrization
            # weights are already in merged format: (K*M, weight_dim)
            x = self.parametrization.forward(
    ...
    def get_luts_and_ids(self):
        ...
        for level in range(self.tree_depth):
            # tree_weights[level] is now (K*lut_rank**i, weight_dim)
            # Pass entire level weights to parametrization
            luts, ids = self.parametrization.get_luts_and_ids(self.tree_weights[level])
            # Convert to list format for compatibility
            level_luts = [luts[i] for i in range(luts.shape[0])]
            level_ids = [ids[i] for i in range(ids.shape[0])]
    ...
    def get_luts(self):
        ...
        for level in range(self.tree_depth):
            # tree_weights[level] is now (K*lut_rank**i, weight_dim)
            # Pass entire level weights to parametrization
            luts = self.parametrization.get_luts(self.tree_weights[level])
            # Convert to list format for compatibility
            level_luts = [luts[i] for i in range(luts.shape[0])]
            tree_luts.append(level_luts)
        return tree_luts
