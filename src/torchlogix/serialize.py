...
def eliminate_dead_gates(circuit: Circuit) -> Circuit:
    """Return a new Circuit with gates unreachable from any output removed."""
    gate_by_id = {g.gate_id: g for g in circuit.gates}

    # Backward BFS from outputs
    visited: set[int] = set()
    queue = list(circuit.output_ids)
    while queue:
        gid = queue.pop()
        if gid in visited or gid < circuit.n_inputs:
            continue
        visited.add(gid)
    ...
    live_gates = [g for g in circuit.gates if g.gate_id in visited]
    return Circuit(
        n_inputs=circuit.n_inputs,
        input_shape=circuit.input_shape,
        gates=live_gates,
        output_ids=circuit.output_ids,
...
def constant_fold_gates(circuit: Circuit) -> Circuit:
    """
    Single forward pass: simplify gates whose inputs are known constants.
    E.g. OR(false, false) → CONST_FALSE, AND(x, true) → WIRE(x).
    Pair with eliminate_dead_gates to remove now-redundant const gates.
    """
...
def emit_lisp(circuit: Circuit, inline_single_use: bool = False) -> str:
...
def emit_c(circuit: Circuit,
           func_name: str = "circuit_eval",
           inline_single_use: bool = False) -> str:
    """
    Emit C code for the circuit.
...
def emit_verilog(circuit: Circuit,
                 module_name: str = "circuit_eval",
                 inline_single_use: bool = False) -> str:
    """
    Emit synthesisable Verilog for the circuit.
    ...
    lines.append(f"// Input shape:  {circuit.input_shape}")
    lines.append(f"// Output shape: {circuit.output_shape}")
    lines.append(f"// n_inputs={n_in}  n_gates={len(circuit.gates)}  n_outputs={n_out}")
    lines.append("")
...
def serialize_circuit(gm: torch.fx.GraphModule,
    ...
    """
    Build a boolean gate circuit from the FX graph and write IR + C files.

    constant_fold_views is applied automatically (it is not optional — see
    the docstring on that function for why).

    Optional optimization passes (all default False for a faithful
    'naive' translation; enable as needed):

      const_fold=False  : algebraic simplification — evaluate gates whose
                         inputs are known constants, e.g. OR(false,x)→x.
                         Run before dce to maximise removal.
      dce=False         : dead-code elimination — remove gates not reachable
                           from any output.
      inline_single_use=False : fold single-use intermediate gates into their
                         parent's expression instead of emitting variables.
                         Applied identically to C, Lisp, and Verilog output.

    verilog_path: if given, also write a synthesisable Verilog file.
    module_name:  Verilog module name (also used as C function name).
    """
    ...
    if const_fold:
        ...
        circuit = constant_fold_gates(circuit)
        # const_fold introduces WIRE / CONST nodes; DCE is most useful after
        if dce:
            circuit = eliminate_dead_gates(circuit)
            print(f"const_fold + DCE: {before} → {len(circuit.gates)} gates "
                  f"(removed {before - len(circuit.gates)})")
        else:
            print(f"const_fold: {before} → {len(circuit.gates)} gates")
    elif dce:
        before = len(circuit.gates)
        circuit = eliminate_dead_gates(circuit)
        removed = before - len(circuit.gates)
        if removed:
            print(f"DCE: removed {removed} dead gates "
                  f"({before} → {len(circuit.gates)})")

    lisp_ir = emit_lisp(circuit, inline_single_use=inline_single_use)
    with open(lisp_path, "w") as f:
        f.write(lisp_ir)

    c_src = emit_c(circuit, func_name=module_name,
                   inline_single_use=inline_single_use)
    with open(c_path, "w") as f:
        f.write(c_src)

    if verilog_path is not None:
        v_src = emit_verilog(circuit, module_name=module_name,
                             inline_single_use=inline_single_use)
